...
def _read_index(path: str | Path):
    ...
    p = Path(path)
    if not p.exists():
        raise FileNotFoundError(f"index.csv not found: {p}")
    df = pd.read_csv(p)

    # robust numeric casting (no FutureWarning, no indentation issues)
    for c in df.columns:
        try:
...
def compare_strict_relaxed(
    ...
    *,
    ...
    plt.xticks(x + (len(labels) - 1) * width / 2, scenarios)
...
def generate_mode_comparison_plots(relaxed_index: str | Path, strict_index: str | Path, out_dir: str | Path) -> Dict[str, Path]:
    ...
    figs["delta_migration_bytes"] = compare_strict_relaxed(
        relaxed_index, strict_index, out_dir,
